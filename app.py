...
SUPABASE_URL = "https://ievhntskoenaqfcunwby.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlldmhudHNrb2VuYXFmY3Vud2J5Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4NDYzMjI0MywiZXhwIjoyMTAwMjA4MjQzfQ.cOH8QUNH8TjdTr-a2yUxCtowzghcbowJyNSKf4RnmHA"
supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
BUCKET_NAME = "fifa-files"
...
def get_db_connection():
    conn = psycopg2.connect(
        host="db.ievhntskoenaqfcunwby.supabase.co",
        database="postgres",
        user="postgres",
        password="Essa12121313$$$$",
        port=5432
    )
...
def download_letter_file(letter_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT file_name, file_path FROM letters WHERE id = %s', (letter_id,))
    row = cursor.fetchone()
    cursor.close()
    conn.close()
    ...
    if row and row['file_path']:
        try:
            public_url = supabase.storage.from_(BUCKET_NAME).get_public_url(row['file_path'])
            return redirect(public_url)
        except Exception as e:
            return f"خطأ أثناء جلب الملف من التخزين السحابي: {str(e)}", 500
...
def download_ach_file(ach_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT file_name, file_path FROM monthly_achievements WHERE id = %s', (ach_id,))
    row = cursor.fetchone()
    cursor.close()
    conn.close()
...
def delete_letter(letter_id):
    ...
    if letter and letter['file_path']:
        try:
            supabase.storage.from_(BUCKET_NAME).remove([letter['file_path']])
        except:
            pass
...
def upload_achievement():
    ...
    if file and file.filename != '':
        ...
        file_path = f"achievements/{dept_id}/{file_name}"
        ...
        try:
            ...
            return f'''<script>alert("خطأ في رفع الملف السحابي: {str(e)}"); window.location.href="/monthly_achievements";</script>'''
        
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO monthly_achievements (dept_id, title, file_name, file_path, uploaded_at)
            VALUES (%s, %s, %s, %s, %s)
        ''', (dept_id, title, file_name, file_path, datetime.now().strftime('%Y-%m-%d %H:%M')))
        conn.commit()
        cursor.close()
        conn.close()
...
def clear_monthly_files(dept_id):
    ...
    for ach in achievements:
        cursor.execute('''
            INSERT INTO letters (title, content, priority, sender_id, receiver_id, file_name, file_path, created_at, archive_dept_id)
...
def login():
    ...
    <head>
        ...
        <style>
            ...
            body { font-family: 'Almarai', sans-serif; background: linear-gradient(135deg, #eaf3ec 0%, #d5e2d8 100%); min-height: 100vh; display: flex; align-items: center; justify-content: center; margin: 0; padding: 20px; }
            .login-card { background: rgba(255, 255, 255, 0.96); backdrop-filter: blur(10px); border-radius: 20px; border: 1px solid rgba(197, 160, 89, 0.2); box-shadow: 0 15px 35px rgba(18, 56, 38, 0.12); width: 100%; max-width: 440px; padding: 2.5rem 2rem; position: relative; overflow: hidden; }
            ...
            .brand-icon-box { width: 70px; height: 70px; background-color: rgba(18, 56, 38, 0.08); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.2rem auto; border: 2px dashed var(--fifa-gold); }
            .brand-icon-box i { font-size: 2.2rem; color: var(--fifa-green); }
            .form-floating > .form-control { border-radius: 10px; border-color: #dbe3df; }
            ...
            .btn-fifa { background-color: var(--fifa-green); color: #ffffff; border-radius: 10px; padding: 0.85rem; font-weight: 700; border: none; width: 100%; transition: all 0.3s ease; }
            .btn-fifa:hover { background-color: var(--fifa-green-hover); color: #ffffff; }
            .login-footer { margin-top: 1.8rem; border-top: 1px solid #edf2f0; padding-top: 1rem; font-size: 0.8rem; color: #7c8a84; }
        </style>
    </head>
    <body>
...
def register():
    ...
    <head>
        ...
        <style>
            ...
            .register-card { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(5px); border-radius: 16px; border: 1px solid #d5e2d8; box-shadow: 0 10px 30px rgba(18, 56, 38, 0.08); width: 100%; max-width: 450px; padding: 2rem; position: relative; overflow: hidden; }
...
<head>
    ...
    <style>
        ...
        .sidebar { width: 260px; background-color: var(--fifa-green-primary); color: #ecf0f1; padding-top: 1rem; flex-shrink: 0; transition: all 0.3s ease; }
        ...
        .section-header { font-weight: 800; color: var(--fifa-green-primary); margin-bottom: 1.5rem; position: relative; padding-bottom: 10px; font-size: 1.4rem; }
        ...
        .btn-fifa-primary { background-color: var(--fifa-green-primary); color: #ffffff; border-radius: 8px; padding: 0.7rem; font-weight: 700; border: none; }
        ...
        <div class="container-fluid">
            <div class="d-flex align-items-center gap-2">
                <button class="btn btn-outline-success d-lg-none py-1 px-2 border-0" type="button" onclick="toggleSidebar()">
                    <i class='bx bx-menu fs-3' style="color: var(--fifa-green-primary);"></i>
                </button>
            ...
            <div class="d-flex align-items-center gap-2 gap-md-3">
                ...
                <span class="text-muted fs-7 d-none lg-inline">{{ now.strftime('%Y/%m/%d') if now else '' }}</span>
                <div class="dropdown">
    ...
    <div class="main-wrapper">
        <aside class="sidebar" id="sidebarMenu">
            <div class="d-flex justify-content-between align-items-center px-3 mb-2 d-lg-none">
                ...
                <button class="btn text-white fs-4" onclick="toggleSidebar()">&times;</button>
            </div>
            ...
            {% if is_admin %}
            <a href="/admin/dashboard" class="sidebar-link mt-2" style="background-color: rgba(197, 160, 89, 0.2);"><i class='bx bxs-cog' style="color: var(--fifa-gold);"></i>لوحة التحكم الشاملة</a>
                            ...
                            {% for letter in letters %}
                                <div class="letter-item d-flex flex-column flex-sm-row align-items-start justify-content-between gap-2">
                                    ...
                                    <div class="d-flex align-items-center gap-2 w-100 justify-content-end mt-2 mt-sm-0">
                                        {% if letter.file_path %}
                                            <a href="/download_letter_file/{{ letter.id }}" target="_blank" class="btn btn-sm btn-outline-success py-1 px-2 fs-7" style="border-color: var(--fifa-green-primary); color: var(--fifa-green-primary);">فتح المرفق</a>
                                        {% endif %}
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content modern-card border-0 shadow-lg">
                <div class="modal-header border-bottom px-4 py-3" style="background-color: var(--fifa-green-primary); color: #fff;">
                    <h5 class="modal-title fw-bold fs-6" id="sendLetterModalLabel"><i class='bx bxs-paper-plane ms-2' style="color: var(--fifa-gold);"></i>قائمة تعبئة وإرسال الخطاب</h5>
                ...
                <div class="modal-body p-4">
                    <form action="/send_letter" method="post" enctype="multipart/form-data">
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            <button type="button" class="btn btn-secondary fs-7 px-4" data-bs-dismiss="modal">إلغاء</button>
...
def send_letter():
    ...
    if file and file.filename != '':
        ...
        file_path = f"letters/{sender_id}/{file_name}"
        file_bytes = file.read()
        ...
        try:
            ...
            return f'''<script>alert("خطأ في رفع الملف السحابي للخطاب: {str(e)}"); window.location.href="/outbox";</script>'''
        
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO letters (title, content, priority, sender_id, receiver_id, file_name, file_path, created_at)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
    ''', (title, content, priority, sender_id, receiver_id, file_name, file_path, datetime.now().strftime('%Y-%m-%d %H:%M')))
    conn.commit()
    cursor.close()
    conn.close()
...
def quick_upload():
    ...
    if request.method == 'POST':
        ...
        for file in files:
            if file and file.filename != '':
                original_name = secure_filename(file.filename)
                file_name = f"{int(datetime.now().timestamp())}_{original_name}"
                file_path = f"quick_uploads/{dept_id}/{file_name}"
    ...
    <head>
        ...
        <style>
            ...
            .upload-card { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(5px); border-radius: 16px; border: 1px solid #d5e2d8; box-shadow: 0 10px 30px rgba(18, 56, 38, 0.08); width: 100%; max-width: 650px; padding: 2rem; position: relative; }
                                ...
                                <div class="p-3">
                                    ...
                                    <form action="/upload_achievement" method="post" enctype="multipart/form-data" class="bg-white p-2 rounded border">
                                        ...
                                        <div class="input-group input-group-sm">
                                            ...
                                            <button class="btn btn-fifa-gold fs-8" type="submit">رفع</button>
                                        </div>
                                    </form>
                                    {% endif %}
                                </div>
...
def admin_dashboard():
    ...
    if request.method == 'POST':
        ...
        content = request.form.get('content', '')
        cursor.execute('''
            INSERT INTO letters (title, content, priority, sender_id, receiver_id, created_at)
            VALUES (%s, %s, %s, %s, %s, %s)
        ''', (title, content, 'عادي', session['dept_id'], session['dept_id'], datetime.now().strftime('%Y-%m-%d %H:%M')))
        conn.commit()
        cursor.close()
        conn.close()
        return '''<script>alert("تمت الإضافة بنجاح عبر لوحة التحكم!"); window.location.href="/admin/dashboard";</script>'''
    ...
    <head>
        ...
        <style>
            :root { --fifa-green-primary: #123826; --fifa-gold: #c5a059; --fifa-bg: #eaf3ec; --fifa-card-border: #d5e2d8; }
            body { font-family: 'Almarai', sans-serif; background-color: var(--fifa-bg); color: #2b302e; overflow-x: hidden; }
            .sidebar { width: 260px; background-color: var(--fifa-green-primary); color: #ecf0f1; padding-top: 1rem; flex-shrink: 0; position: fixed; top: 0; right: 0; height: 100vh; z-index: 1050; }
            ...
            .sidebar-link:hover, .sidebar-link.active { background-color: rgba(255, 255, 255, 0.08); color: #ffffff; border-right-color: var(--fifa-gold); font-weight: 700; }
            .sidebar-link i { font-size: 1.35rem; margin-left: 12px; color: var(--fifa-gold); }
            .main-content { margin-right: 260px; padding: 1.5rem; }
            .modern-card { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(5px); border-radius: 12px; border: 1px solid var(--fifa-card-border); box-shadow: 0 4px 15px rgba(18, 56, 38, 0.03); }
    ...
    <body>
        <aside class="sidebar">
            <div class="text-center px-3 mb-3">
                <h5 class="fw-bold text-white fs-6">نادي فيفا الرياضي</h5>
                <small class="text-warning fs-8">لوحة التحكم الشاملة</small>
            </div>
            <a href="/dashboard" class="sidebar-link"><i class='bx bxs-inbox'></i>الصندوق الوارد</a>
            <a href="/outbox" class="sidebar-link"><i class='bx bxs-paper-plane'></i>الخطابات الصادرة</a>
            <a href="/monthly_achievements" class="sidebar-link"><i class='bx bxs-trophy'></i>إنجازات الشهر</a>
            <a href="/archive" class="sidebar-link"><i class='bx bxs-file-archive'></i>أرشيف الإدارة</a>
            <a href="/admin/dashboard" class="sidebar-link active"><i class='bx bxs-cog' style="color: var(--fifa-gold);"></i>لوحة التحكم الشاملة</a>
            <a href="/admin/permissions" class="sidebar-link"><i class='bx bxs-shield'></i>إدارة الصلاحيات</a>
            <a href="/logout" class="sidebar-link text-danger"><i class='bx bx-log-out text-danger'></i>تسجيل الخروج</a>
        </aside>
        <div class="main-content">
            <div class="modern-card p-4 mb-4">
                <h5 class="fw-bold fs-6 mb-3" style="color: var(--fifa-green-primary);">إضافة عنصر أو معاملة جديدة</h5>
                <form action="/admin/dashboard" method="post">
                    <div class="mb-3">
                        <label class="form-label fw-bold fs-7">عنوان العنصر / الخطاب</label>
                        <input type="text" name="title" class="form-control fs-7" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold fs-7">الوصف أو التفاصيل</label>
                        <textarea name="content" class="form-control fs-7" rows="3"></textarea>
                    </div>
                    <button type="submit" class="btn btn-fifa-gold fs-7 py-2 px-4">حفظ الإضافة</button>
                </form>
            </div>
            <div class="modern-card p-3">
                <h5 class="fw-bold fs-6 mb-3 px-2">قائمة العناصر والخطابات المسجلة</h5>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="/delete_letter/{{ item.id }}" class="btn btn-sm btn-outline-danger py-1 px-2 fs-8" onclick="return confirm('حذف؟');">حذف</a>
                                </td>
                            </tr>
                            {% endfor %}
                        </tbody>
...
def admin_permissions():
    ...
    <body>
        <aside class="sidebar">
            <div class="text-center px-3 mb-3">
                <h5 class="fw-bold text-white fs-6">نادي فيفا الرياضي</h5>
                <small class="text-warning fs-8">إدارة الصلاحيات والمستخدمين</small>
            </div>
            <a href="/dashboard" class="sidebar-link"><i class='bx bxs-inbox'></i>الصندوق الوارد</a>
                    ...
                    <table class="table table-bordered align-middle fs-7 bg-white">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="/admin/delete_department/{{ dept.id }}" class="btn btn-sm btn-outline-danger py-1 px-2" onclick="return confirm('هل أنت متأكد من حذف هذا المستخدم/الإدارة نهائياً؟');">حذف اليوزر</a>
                                </td>
                            </tr>
                            {% endfor %}
                        </tbody>
